...
class JSONMapper {
    ...
    map (key_pairs, json_file_or_object=null) {
        if (json_file_or_object !== null) {
            this._getObject(json_file_or_object);
        }
        let formedObject = {};
        ...
        key_pairs.forEach((pair) => {
            if (Array.indexOf(Object.keys(this.json_object), pair[0])) {
                destination[pair[1]] = this.json_object[pair[0]];
            }
        });
        return formedObject;
    }
