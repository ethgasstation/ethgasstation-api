...
const DEFAULT_INI_LOCATIONS = [
        ...
        '/opt/ethgasstation/api/settings.conf'
    ];
...
class EGSSettings {
     ...
     */
    getSettingsFilepath() {
        DEFAULT_INI_LOCATIONS.forEach((candidateLocation) => {
            if (fs.existsSync(candidate_location)) {
                return path.normalize(candidate_location);
            }
        });
    }
...
module.exports = new EGSSettings(false);